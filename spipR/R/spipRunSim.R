...
#' @param sleep_time sleep time at the end of a Spip run. In order to ensure 
#'   different seeds between runs, it should be greater or equal to 3.
#'
...
#' seeds = unlist(lapply(sims, function(x) {
...
spipRunSim = function(ref_genotypes,
  ...
  warning("\"spip_seeds\" was removed (if any file like this existed). This means Spip will use the clock to initialize the seeds. This might cause problems if Spip is run simultaneously or in a very short time (similar seeds between simulations). Using a sleep_time greater or equal to 3 might be enough.")
